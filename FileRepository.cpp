...
void FileRepository::scrieCalatoriiDinVectorInFisier(std::vector<Calatorie> calatorii)
{
	std::ofstream fout(writeFileName);
	for (int i = 0; i < calatorii.size(); ++i)
		fout << "Cod: " << calatorii[i].getCod() << " Plecare: " << calatorii[i].getLocalitatePlecare() << " Destinatie: " << calatorii[i].getLocalitateDestinatie() << " Data: " << calatorii[i].getDataPlecarii() << " Nr.Locuri Totale:" << calatorii[i].getNrLocTotale() << " Nr.Locuri Rezervate: " << calatorii[i].getNrLocRezervate() << '\n';
	fout.close();
}
